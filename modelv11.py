...
def run(weights, source, savedir, imgsz=640, conf=0.25):
    ...
    results = model(source, imgsz=imgsz, conf=conf, verbose=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--weights', type=str, required=True, help='YOLOv11 model weights path')
    parser.add_argument('--image', type=str, required=True, help='Input image path')
    parser.add_argument('--savedir', type=str, default='results', help='Save directory')
    parser.add_argument('--imgsz', type=int, default=640, help='Image size')
    parser.add_argument('--conf', type=float, default=0.25, help='Confidence threshold')

    args = parser.parse_args()
    run(args.weights, args.image, args.savedir, args.imgsz, args.conf)